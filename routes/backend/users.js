...
router.get('/', async (req, res, next) => {// khi truyền dữ liệu qua đường dẫn để lấy được dữ liệu đấy ta thêm /:status vào router. Nếu ko có dữ liệu truyền trên đường dẫn thì thêm (/:status)? nghĩa là có ý nghĩa là chuỗi được gửi lên có cũng được ko có cũng được
    ...
    //         usersModel.updateOne({_id: req.query.id}, {status: "inactive"}, (err, affected, res)=>{
    //             console.log("success");
    //         })
    //     }else{
    //         usersModel.updateOne({_id: req.query.id}, {status: "active"}, (err, affected, res)=>{
    //             console.log("success");
    //         })
    //     }
    // }
...
router.get('/add(/:id)?', function(req, res, next) {
    ...
    if (req.params.id === undefined) {
        res.render(`inc/admin/${col}/add`, {layout: 'admin', title: 'add page', data, validatorErr, baselink, col });
    } else {
        
        var data = {};
        usersModel.findById(req.params.id, (err, result) => {
            data = result;
            res.render(`inc/admin/${col}/add`, {layout: 'admin', title: 'edit page', data, validatorErr, baselink, col });
        });
